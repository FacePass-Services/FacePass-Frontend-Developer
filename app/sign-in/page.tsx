...
const App = () => {
  ...
  const sendFrameToFlask = useCallback(async () => {
  ...
  const faceLiveness = useCallback(
    (detectedOrientation: string) => {
      ...
      if (
        isNotNull(randomOrientation) &&
        detectedOrientation.toLowerCase() === randomOrientation.toLowerCase()
      ) {
  ...
  // Wrap detectFace in useCallback
  ...
  }, [faceDetected, findingFaceStartTime, findingFace, stopCamera]);
  ...
  }, [isFaceLiveness, videoStream, faceDetected, detectFace, sendFrameToFlask]);
